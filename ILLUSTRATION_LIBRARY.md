...
> **Style lock:** *"Painterly watercolor-style illustration with soft gradients for depth, warm directional lighting, semi-realistic architectural proportions (not chibi, not photorealistic). Ink-indigo (#2B3868) linework. Palette limited to sky blue, teal, sakura pink, warm orange used sparingly, and warm cream in day scenes; deep indigo, gold, and warm lantern-orange in night scenes. Sakura branches may frame one or two corners but never cover the subject. No text, no UI chrome baked into the image."*

**Updated 2026-07-15** against `design-reference/visual-dna-reference.png` (now the visual source of truth, see `DESIGN_SYSTEM.md` §0.1): dropped the old "no neon, no dark/moody lighting" restriction. A Ninja-mode (night) scene — Dotonbori's neon signage, a lantern-lit street at dusk — is warm and inviting, not moody/cyberpunk, and the reference treats that glow as core to the Japan-at-night feeling, not something to avoid. The distinction that still matters: warm lantern/neon glow (yes) vs. cold cyberpunk/gaming RGB (still no).
