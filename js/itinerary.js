...
function renderDaySelector() {
    document.getElementById('daySelector').innerHTML = ITINERARY_DATA.map(day => `
        <button data-day="${day.day}" class="day-btn px-4 py-2 rounded-lg whitespace-nowrap font-medium ${currentDay === day.day ? 'bg-red-600 text-white shadow-md' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300'}">
            Día ${day.day}
        </button>
    `).join('');
}
...
function renderDayOverview(day) {
    ...
    document.getElementById('dayOverview').innerHTML = `
        ...
        <div class="space-y-3 text-sm">
            <p class="font-semibold text-base">${day.date}</p>
...
function renderActivities(day) {
    document.getElementById('activitiesTimeline').innerHTML = day.activities.map((act, i) => `
