...
export const ItineraryBuilder = {
  ...
  async generateActivitiesFromTemplate(templateId, cityDayAssignments, selectedCategories, totalDays, startDate) {
    ...
    cityDayAssignments.forEach(dayAssignment => {
      ...
      cities.forEach((cityVisit, cityIndex) => {
        ...
        if (selectedActivities.length === 0) {
          ...
          // Filter activities by selected categories (handles both .category string and .categories array)
          let availableActivities = cityData.activities.filter(activity => {
            if (Array.isArray(activity.categories)) {
              return activity.categories.some(cat => allCategories.includes(cat));
            }
            return allCategories.includes(activity.category);
          });
        ...
        selectedActivities.forEach((activity, i) => {
          ...
          generatedActivities.push({
            ...
            ...activity,
            // city/cityId must come after spread so they're never overwritten by activity.city
            city: cityId,
            time: cityVisit.timeStart
              ? this.calculateActivityTime(currentTime, i, activity, selectedActivities.length)
              : activity.time || `${String(hour).padStart(2, '0')}:00`,
            cityVisitInfo: {
  ...
  calculateActivityTime(startTime, index, activity = null, totalActivities = null) {
    ...
    if (activity) {
      ...
      // Si no, estimar basado en categoría
        ...
        const actCat = activity.category || (Array.isArray(activity.categories) ? activity.categories[0] : null);
        activityDuration = durationByCategory[actCat] || 90;
      }
    }
