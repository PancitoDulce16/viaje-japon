...
## 2026-07-15 — Reverted: "Concept A — Full-Bleed Horizon" minimalist hero composition

**What was removed:** An in-progress, never-committed rewrite of `css/dashboard-hero.css`, `js/features/dashboard/hero-moment.js`, `js/features/trips/trips-manager.js`'s quick-actions markup, and `dashboard.html`'s wallpaper background. It replaced the trip-countdown hero with a short full-bleed illustration that dissolved into a flat page background, moved copy below the image in plain ink-on-cream text, swapped the four large action buttons for quiet 38px icon circles, and reduced the wallpaper to a flat color with a faint gradient wash. Never deployed — caught and reverted at proposal-review stage, before a second push to production.

**What replaced it:** Nothing yet — the four files were `git checkout`'d back to their last-committed state (the Slice 1–4 Dashboard Experience hero, scrim-over-image with white text). A new composition proposal is expected next, this time required to pass `DESIGN_SYSTEM.md` §0.1 (Kawaii Day / Ninja Night) before implementation, informed by the reference board at `design-reference/visual-dna-reference.png`.

**Why:** It was reviewed against real mobile *and* desktop screenshots and failed the "hide the logo, is it still Japitin?" test — see `DESIGN_SYSTEM.md` §0.1, added in direct response to this rejection. Concretely: it read as a generic modern dashboard in light mode (lost the Kawaii personality entirely), lost the Ninja personality in dark mode, the illustration shrank to what felt like decoration rather than branding, and the desktop composition specifically was called out as visually empty and unbalanced — a stretched mobile layout, not a composition designed for the wider canvas. The countdown hero — the emotional center of the screen — read as an informational banner instead. None of these are things `npm run build` or Playwright contrast/console checks can catch; they only surfaced from a human looking at real screenshots against the product's actual identity.

**Migration notes:** No user data involved — CSS/markup only, never deployed. One real, direction-independent bug was found and *kept* during this work (not reverted): `.hero-moment__cta` and the new `.dash-quick-btn` class were silently losing their intended colors to blanket, high-specificity/`!important` legacy rules — `button:not(.floating-btn):not([class*="bg-"]):not(.sos-button):not(.header-chip)` in `css/theme-kawaii.css` (light) and its `.dark` twin in `css/theme-ninja.css`; `html.dark button:not([class*="bg-red"])...` in both `css/main.css` and `css/dark-mode-ultra-contrast.css`; a blanket `html.dark *`/`html.dark p` white-text rule in `css/dark-mode-ultra-contrast.css`; and a mobile `min-height:48px !important` touch-target rule in `css/mobile-optimizations.css` that was stretching the quiet icon circles into ovals. All five now carry `:not(.hero-moment__cta):not(.dash-quick-btn)` exclusions (following the existing `.sos-button`/`.header-chip` precedent already in those files). This is orthogonal to which visual direction wins — any future custom-styled button using these two class names needs this to render its own colors at all, Kawaii/Ninja-faithful or not.

**Process note:** This is the second time in two days a real defect only surfaced once a human looked at the actual rendered page rather than trusting computed-style assertions or dev-server screenshots reviewed in isolation (see the entry below). Contrast ratios and "no console errors" are necessary checks, not sufficient ones — they can't tell you a screen stopped looking like the product.
